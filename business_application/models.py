...
class Incident(NetBoxModel):
    ...
    def get_recent_context_events(self, limit=20):
        """
        Get recent non-incident events for devices and VMs affected by this incident.
        These events provide context but are not assigned to the incident.
        """
        from django.contrib.contenttypes.models import ContentType
        from dcim.models import Device
        from virtualization.models import VirtualMachine

        # Early return if no affected services
        if not self.affected_services.exists():
            return Event.objects.none()

        # Get all devices and VMs from affected services
        device_ids = set()
        vm_ids = set()

        for service in self.affected_services.all():
            device_ids.update(service.devices.values_list('id', flat=True))
            vm_ids.update(service.vms.values_list('id', flat=True))

        # Early return if no devices or VMs found
        ...
        # Build query for events from affected infrastructure
        ...
        events_query = Event.objects.exclude(
            ...
            # Exclude events already assigned to this incident
            incidents=self
